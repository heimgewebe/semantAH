...
[[ "$age_days" =~ ^[0-9]+$ ]] || { echo "BACKUP_AGE_DAYS muss eine nichtnegative Zahl sein (got: '$age_days')" >&2; exit 1; }

if date -d "@0" +%s >/dev/null 2>&1; then
  # GNU date
  last_ok=$(date -d "${age_days} day ago" +%F)
elif date -v-"${age_days}"d +%F >/dev/null 2>&1; then
  # BSD/macOS date
  last_ok=$(date -v-"${age_days}"d +%F)
else
  echo "weder GNU date (-d) noch BSD date (-v) verfügbar" >&2
  exit 1
fi
